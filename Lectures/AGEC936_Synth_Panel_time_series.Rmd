...
subtitle: "Panel Time Series<br>Synthetic Control Methods"
...
# Time-series considerations in Panel data setting

# Synthetic control methods
* Foundational pieces: @abadie2010synthetic and @abadie2019using
...
* Under certain conditions, we have $\sum_{j=2} w^{*}_j y_{jt}$ approximately equal to $y_{1t}(0)$. Thus, we can use
  ...
  $$
* Estimation (see equation (7) of @abadie2019using):
  $$
  Min_{w_j,j\neq1}\;\left(\sum_h v_h \left(X_{1h}-\sum_{j=2} w_j X_{2j}\right)^2\right)
  $$
  subject to
  $$
...
tempfile temp
synth y y(1) y(2) y(3) y(4) y(5) x(1(1)5), trunit(1) trperiod(10) mspeperiod(1(1)7) keep(`temp')
...
* **Background**
  ...
  4. Sample period: 1955 - 1997 (pre-treatment period: 1955 - 1969, optimization period: 1960 - 1969)
  5. Uses 12 predictors
...
# Different optimiztion period
* What if we use 1955 - 1964 instead of 1960 - 1969?
...
# Synthetic control: Strengths and things to consider [@abadie2019using]
* See the latest review of @abadie2019using.
* Strengths
...
* Further readings
  + Flexibility versus extrapolation risks
    ...
    - Non-negative, weights sum to one, but allow parallel shifts (a constant can be added or subtracted): @goh2020synthetic
  + Innovations in inference techniques
    - Frequentist approaches: e.g. @li2020statistical, @chernozhukov2017exact
    - Bayesian approaches: e.g. @amjad2018robust, @goh2020synthetic
  + Extension to disaggregated data: @arkhangelsky2019synthetic, @lhour2020apenalized
  + De-bias approach: @ben2018augmented
