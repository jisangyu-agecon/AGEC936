...
Y_1 <- 5+0.2*X+0.3*rnorm(N)
# Post
Y_2 <- 5+2*(d+1)*X+0.2*X+tau*d+0.3*rnorm(N)
...
# @de2020two $DID_M$ estimator: simulation example
...
* **Estimation**
  ...
  + Definition of **Doubly Robust**?

* **Aggregation**: It depends on what you are interested in. For example, are you interested in the average of static effects *with-in* group? Or are you interested in the *dynamic* effects measured by the lengeth of *exposure*. See table 1 of @callaway2020difference for the discussion on "weights".

# @callaway2020difference: An example
...
* When?: (Normally), **one** treated unit ($i=1$), **many** control units, and (somewhat) long time period.
* Back to **potential outcome representation**: we want
$$
\tau_{t}=y_{1t}(1)-y_{1t}(0)
$$
* What do we observe?: $y_{1t}=y_{1t}(0)$ if $t<t^*$ and $y_{1t}=y_{1t}(1)$ if $t>t^*$. And $y_{it}=y_{it}(0)$ for $i\neq1$.
...
* **Background**
  ...
  4. Sample period: 1955 - 1997 (pre-treatment period: 1955 - 1969)
...
* Great summary and discussion by @abadie2019using
